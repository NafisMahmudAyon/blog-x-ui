    ...
    <>
      <Blog className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 lg:gap-4'>
        {posts.slice(0, 7).map((post, index) => {
          return (
            <Blog key={index} className={`relative text-white ${index == 0 ? "md:col-start-1 md:col-end-3" : ""} `}>
              <BlogAction link={post.link} className='absolute inset-0 z-10' />
              <FeaturedImage src={`${index == 0 ? "https://via.placeholder.com/1200x400" : "https://via.placeholder.com/600x400"}`} alt='image' className='object-cover h-full' />
